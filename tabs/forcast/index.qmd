---
title: "Alewife Telemetry - Lake Ontario - Forcast Temperatures"
date: "Updated: `r Sys.Date()`"
---
# Purpose
This site is designed to provide quick updates on projected water temperatures 
...
base_url <- 'https://www.glerl.noaa.gov/emf/ont/fcast/'
# first we grab the current time to download the latest push 
current_time <- paste0(format(Sys.Date() - 1, "%m%d"),
...
temps_joined <- temps_joined %>% 
  arrange(node, time) %>% 
  mutate(
    sig = rep(rep(seq(0.025, 0.975, 0.05), 72), nrow(locs_select)), 
    depth = h * sig
  )
```
...
Below are predicted thermal profiles for nodes `3383`, `4512`, and `5088` which are the center of the the array. The frist plot is static. 
...
p <- ggplot(data = temps_joined %>% 
  ...
  labs(
    x = "Date",
    y = "Depth (m)", 
    title = paste("Forcast start date:", current_time, sep = " ")
  )
```
