---
title: "Alewife Telemetry - Lake Ontario - Now Temperatures"
...
temps_joined <- temps_joined %>% 
  arrange(node, time) %>% 
  mutate(
    sig = rep(rep(seq(0.025, 0.975, 0.05), 12), nrow(locs_select)), 
    depth = h * sig
  )
```
...
p <- ggplot(data = temps_joined %>% 
  ...
  labs(
    x = "Date",
    y = "Depth (m)", 
    title = paste("Nowcast start date:", current_time, sep = " ")
  )
```
